...
const QuizDetails: React.FC<QuizDetailsProps> = ({ visible, quiz, onClose }) => {
  if (!quiz) return null;
  ...
  return (
    ...
    >
      <SafeAreaView style={{ backgroundColor: "rgba(0, 0, 0, 0.5)", flex: 1 }} className="justify-center items-center px-4">
        <View className="bg-white w-[90%] rounded-2xl" style={{ maxHeight: '90%', minHeight: '70%' }}>
          {/* Header */}
          <View className="flex-row justify-between items-start p-6 border-b border-gray-200">
            <View className="flex-1 mr-4">
              <Text className="text-3xl font-semibold text-gray-800 mb-2">{quiz.title}</Text>
              <View className="flex-row items-center mb-2">
                <View className="bg-blue-100 px-3 py-1 rounded-full mr-3">
                  <Text className="text-blue-700 font-medium text-sm">{quiz.type}</Text>
                </View>
          ...
          >
            ...
            {quiz.description && (
              <View className="mb-6">
                ...
                <View className="bg-gray-50 p-4 rounded-lg">
                  <Text className="text-gray-700 leading-6">{quiz.description}</Text>
                </View>
              </View>
            )}
            ...
            {/* Questions Section */}
            <View className="mb-4">
              <Text className="text-lg font-semibold text-gray-800 mb-4">Questions</Text>
              
              {quiz.questions.map((q: any, index: number) => (
                <View key={index} className="mb-6 bg-gray-50 rounded-xl p-5 border border-gray-200">
                  {quiz.type === "Multiple Choice" ? (
                    <>
                      ...
                      <View className="flex-row items-center mb-3">
                        ...
                        <Text className="text-lg font-semibold text-gray-800 flex-1 mt"                         
                        style={{ marginTop: 12 }}>
                          {q.question}
                        </Text>
                      </View>
                      ...
                      {q.videoUrl && (
                        <View className="mb-4 bg-purple-50 p-3 rounded-lg border border-purple-200">
                          <Text className="text-purple-700 font-medium text-sm mb-1">Video Attached</Text>
                          <Text className="text-purple-600 text-xs" numberOfLines={1}>
                            {q.videoUrl}
                          </Text>
                        </View>
                      )}
                    ...
                    {(['A', 'B', 'C', 'D'] as const).map((choice) => (
                      ...
                      >
                        <Text 
                          style={q.correctAnswer === choice ? { color: 'white' } : { color: 'black' }}
