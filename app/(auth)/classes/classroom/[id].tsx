...
import { Modal, TextInput, View, Text, TouchableOpacity, ScrollView, Image } from "react-native";
...
const Classroom = () => {
  ...
  const handleAddStudents = (newStudents) => {
    const newUniqueStudents = newStudents.filter(newStudent =>
      ...
      <ScrollView className="w-full">
        {students.map((student) => (
          ...
          >
            ...
            <TouchableOpacity onPress={() => handleRemoveStudent(student.id, student.name)}>
              <Ionicons name="person-remove" size={20} color="#F2A800" /> {/*color="#F2A800" - darker color of yellow*/}
            </TouchableOpacity>
      ...
      <View className="flex-1 p-4">
        <TouchableOpacity className="bg-white rounded-xl p-4 mb-4 shadow-sm border border-gray-100 flex-row items-center justify-between">
          <View className="flex-row items-center">
            ...
            <Text className="text-lg font-poppins-medium text-gray-800">Progress Report</Text>
          </View>
  ...
  {activeTab === 'Students' && (
  <View className="absolute bottom-4 right-4 p-4" style={{ zIndex: 10 }}>
  {/* <View className="absolute bottom-4 right-1 p-4"> */}
      <TouchableOpacity 
        ...
        <Text className="text-white mr-2 font-poppins-medium">Add student</Text>
  ...
  <View className="absolute bottom-4 right-1 p-4" style={{ zIndex: 100 }}>
  <TouchableOpacity 
        className="bg-primary flex-row items-center rounded-full px-4 py-2 shadow-md"
      ...
      >
        <Text className="text-white mr-2 font-poppins-medium">Assign Activity</Text>
        <Ionicons name="add" size={18} color="white" />
      </TouchableOpacity>
    </View>
  )}
