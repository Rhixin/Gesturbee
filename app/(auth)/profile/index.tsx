    ...
    <>
      ...
      >
        <View className="flex-1 justify-end bg-black/30">
          <View className="bg-primary rounded-t-3xl px-6 pt-4 pb-8 h-[60%] shadow-lg">
            <Pressable onPress={() => setModalVisible(false)} className="self-end mb-2">
      ...
      <SafeAreaView className="absolute top-0 left-0 right-0 z-20 bg-primary shadow px-6 pt-6">
        <View className="flex-row justify-between items-center">
          {[{ icon: 'trophy', count: trophyCount }, { icon: 'flame', count: fireCount }].map((item, index) => (
            <View key={index} className="flex-row items-center px-3 rounded-full">
              <Ionicons name={item.icon} size={20} color="white" />
              <Text className="text-white ml-1 font-semibold">{item.count}</Text>
            </View>
          ))}
        </View>
      </SafeAreaView>
      ...
      {/* Scrollable Content */}
      <ScrollView contentContainerStyle={{ paddingTop: 120, paddingBottom: 30 }}>
        ...
        <View className="bg-[#fff0c2] rounded-2xl p-4 mt-6 w-[90%] self-center items-center shadow-md">
          ...
          <Text className="text-lg font-bold mt-2 mb-3 text-black">Daily Streak</Text>
        ...
        <View className="w-[90%] mt-10 self-center">
          <Text className="text-lg font-poppins-bold mb-4 text-black">Account</Text>

          <View className="space-y-3 " >
            {[{ icon: "person-outline", label: "Edit Personal Information", component: <EditPersonalInformation /> },
              { icon: "lock-closed-outline", label: "Change Password", component: <ChangePassword /> },
              { icon: "settings-outline", label: "Settings", component: <Settings /> }].map((item, index) => (
              <Pressable
                key={index}
              ...
              >
                ...
                <Text className="ml-4 text-medium font-poppins-medium  text-black">{item.label}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View className="w-[90%] mt-10 self-center">
          <Pressable className="bg-red-500 rounded-xl py-4 px-4 shadow" onPress={() => {}}>
    ...
    <>
      {streakData.map((active, idx) => (
        <View key={idx} className="items-center">
          <View
            className={`w-6 h-6 rounded-full ${active ? "bg-primary" : "border-2 border-primary"}`}
          />
